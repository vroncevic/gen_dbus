...
'''
 ...
 Info
     Defined class GenDBus with attribute(s) and method(s).
...
class GenDBus(CfgCLI):
    '''
        ...
        It defines:
            ...
            :methods:
                ...
                | __str__ - dunder method for GenDBus.
    '''

    GEN_VERBOSE = 'GEN_DBUS'
    CONFIG = '/conf/gen_dbus.cfg'
    LOG = '/log/gen_dbus.log'
    LOGO = '/conf/gen_dbus.logo'
    ...
    def __init__(self, verbose=False):
        ...
        gen_dbus_property = {
            ...
            'ats_repository': 'gen_dbus',
            'ats_name': 'gen_dbus',
            'ats_logo_path': '{0}{1}'.format(current_dir, GenDBus.LOGO),
            'ats_use_github_infrastructure': True
        }
        splash = Splash(gen_dbus_property, verbose=verbose)
        base_info = '{0}{1}'.format(current_dir, GenDBus.CONFIG)
        CfgCLI.__init__(self, base_info, verbose=verbose)
        verbose_message(GenDBus.GEN_VERBOSE, verbose, 'init tool info')
        self.logger = ATSLogger(
            GenDBus.GEN_VERBOSE.lower(),
            '{0}{1}'.format(current_dir, GenDBus.LOG),
            verbose=verbose
        )
        if self.tool_operational:
            self.add_new_option(
                GenDBus.OPS[0], GenDBus.OPS[1],
                dest='gen', help='generate option'
            )
            self.add_new_option(
                GenDBus.OPS[2], GenDBus.OPS[3],
                action='store_true', default=False,
                help='activate verbose mode for generation'
            )
            self.add_new_option(
                GenDBus.OPS[4], action='version', version=__version__
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_operational:
            ...
            if num_of_args_sys > 1:
                ...
                if operation not in GenDBus.OPS:
                    sys.argv.append('-h')
            else:
                sys.argv.append('-h')
            ...
            if not pro_exists:
                if bool(getattr(args, 'gen')):
                    print(
                        '{0} {1} [{2}]'.format(
                            '[{0}]'.format(GenDBus.GEN_VERBOSE.lower()),
                            'generating', getattr(args, 'gen')
                        )
                    )
                    generator = DBus(verbose=verbose)
                    status = generator.gen_setup(
                        getattr(args, 'gen'), verbose
                    )
                    if status:
                        success_message(GenDBus.GEN_VERBOSE, 'done\n')
                        ...
                        error_message(
                            GenDBus.GEN_VERBOSE, 'generation failed'
                        )
                        self.logger.write_log(
                    ...
                    error_message(
                        GenDBus.GEN_VERBOSE, 'provide project name'
                    )
                    self.logger.write_log(
                ...
                error_message(
                    GenDBus.GEN_VERBOSE, 'project already exist'
                )
                self.logger.write_log(
            ...
            error_message(
                GenDBus.GEN_VERBOSE, 'tool is not operational'
            )
            self.logger.write_log(
